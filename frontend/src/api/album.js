...
// 4.4 上传照片到相册
// 前端流程：先经 /api/files/upload 逐张上传拿到 url 数组，再绑定到相册
// （与前端任务描述一致：uploadPhotos(id, urls, description)）
export function uploadPhotos(id, urls, description) {
  return request.post(`/albums/${id}/photos`, { urls, description })
}

// 4.5 删除照片（只能删自己上传的，否则 403）
